...
interface PopularChineseNovelsProps {
    onLNClick: (id: string, autoRead?: boolean, lnData?: LightNovel) => void;
}

export default function PopularChineseNovels({ onLNClick }: PopularChineseNovelsProps) {
    ...
    useEffect(() => {
        let mounted = true;
        lnService.getPopularChineseNovels()
            .then((data) => {
                if (mounted) {
                    setNovels(data);
                    setLoading(false);

...
const SpotlightCover: React.FC<{ thumbnail: string; title: string }> = ({ thumbnail, title }) => {
    ...
    return (
        <div
            ref={cardRef}
            className={`hidden md:block w-56 lg:w-64 shrink-0 rounded-xl relative perspective-1000 transition-transform duration-500 ease-out ${
        ...
        >
            <div
                className="w-full h-full rounded-xl overflow-hidden transition-all duration-75 ease-out shadow-2xl"
                style={{
...
export default function LNSpotlight({ onLNClick }: LNSpotlightProps) {
            ...
            <div className="absolute inset-0 overflow-hidden" ref={emblaRef}>
                <div className="flex h-full touch-pan-y">
                    {lns.map((ln, index) => {
                        ...
                        return (
                            <div key={ln.id || index} className="relative min-w-full h-full flex-[0_0_100%]">
            ...
            <div className="absolute inset-0 flex items-center z-10 pointer-events-none">
                <AnimatePresence>
                    {lns[selectedIndex] && (() => {
                        ...
                        const displayTitle = getDisplayTitle(activeLN, language);
                        const cover = activeLN.images.jpg.large_image_url || activeLN.images.jpg.image_url;
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className="flex-1 pointer-events-auto w-full max-w-2xl flex flex-col justify-end h-[360px] md:h-[380px]">
                                    <div className="w-full mb-4">
                                        ...
                                        <div className="flex items-start">
                                            <AnimeLogoImage
                                                tmdbId={parseInt((activeLN.id || '0').toString())}
                                                title={displayTitle}
                                ...
                                {/* Coverflow Stacked Images (Right) */}
                                <div className="ml-auto lg:mr-12 xl:mr-20 pointer-events-none relative w-56 lg:w-64 h-[336px] lg:h-[384px] group">
                                    {/* Previous Card */}
                                    {lns.length > 1 && (
                                        <div
                                            onClick={() => scrollTo((selectedIndex - 1 + lns.length) % lns.length)}
                                            className="absolute inset-0 hidden md:block pointer-events-auto cursor-pointer z-0"
                                        >
                                            <div className="w-full h-full origin-bottom transition-transform duration-500 ease-out [transform:translateX(-40%)_translateY(-20px)_scale(0.9)_rotate(-8deg)] group-hover:[transform:translateX(-45%)_translateY(-20px)_scale(0.92)_rotate(-6deg)]">
                                                <div className="w-full h-full rounded-xl overflow-hidden brightness-[0.6] transition-all duration-300 shadow-xl">
                                                    <img
                                                        src={lns[(selectedIndex - 1 + lns.length) % lns.length].images.jpg.large_image_url}
                                    ...
                                    <div className="absolute inset-0 z-10 pointer-events-auto transition-transform duration-500 ease-out group-hover:-translate-y-4">
                                        {activeLN.images?.jpg?.large_image_url && (
                                            <SpotlightCover thumbnail={activeLN.images.jpg.large_image_url} title={displayTitle} />
                                        )}
                                    </div>
                                    ...
                                    {lns.length > 2 && (
                                        <div
                                            onClick={() => scrollTo((selectedIndex + 1) % lns.length)}
                                            className="absolute inset-0 hidden md:block pointer-events-auto cursor-pointer z-0"
                                        >
                                            <div className="w-full h-full origin-bottom transition-transform duration-500 ease-out [transform:translateX(40%)_translateY(-20px)_scale(0.9)_rotate(8deg)] group-hover:[transform:translateX(45%)_translateY(-20px)_scale(0.92)_rotate(6deg)]">
                                                <div className="w-full h-full rounded-xl overflow-hidden brightness-[0.6] transition-all duration-300 shadow-xl">
                                                    <img
                                                        src={lns[(selectedIndex + 1) % lns.length].images.jpg.large_image_url}
                                                        className="w-full h-full object-cover"
                                                        alt=""
                                                    />
                                                </div>
                ...
                {lns.map((_, idx) => (
                    <button
                        ...
                        className={`w-2 h-2 rounded-full transition-all duration-300 ${
                            idx === selectedIndex ? 'bg-amber-400 md:w-6 h-6 md:h-2' : 'bg-white/30 hover:bg-white/50'
                        }`}
